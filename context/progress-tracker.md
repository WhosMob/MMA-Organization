...
- Implement Rankings page, Events page, News page, About page, Contact page (and `/events/[event-slug]`, `/news/[news-slug]` detail pages) as placeholders or full implementations so nav/footer links resolve.
...
- The `/events/[event-slug]` links emitted by fight rows (e.g. `/events/mma-39-homecoming`, `/events/night-of-champions`) will 404 until the Events pages are built, same as the existing `/news/[news-slug]` pattern.
